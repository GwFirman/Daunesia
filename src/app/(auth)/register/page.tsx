...
export default function Register() {
				...
				fetchOptions: {
					...
					onError: (ctx) => {
						...
						toast.error("Gagal membuat akun", {
							description: ctx.error?.message || "Terjadi kesalahan saat membuat akun",
						});
	...
	const handleSocialLogin = async (provider: "google" | "facebook") => {
		...
		const loadingToast = toast.loading(`Menghubungkan ke ${provider === "google" ? "Google" : "Facebook"}...`, {
					...
					fetchOptions: {
						onError: () => {
							...
							toast.error("Gagal mendaftar", {
								description: `Tidak dapat mendaftar dengan ${provider === "google" ? "Google" : "Facebook"}`,
							});
							setLoadingProvider(null);
						},
					},
								...
								<div className="flex w-full max-w-lg flex-1 flex-col gap-4">
									<div className="flex flex-col gap-1">
										...
										<motion.div animate={{ y: [-12, 0], opacity: [0, 1] }} transition={{ duration: 1.3, delay: 0.2 }} className="text-font-secondary">
											Gabung sekarang untuk mendeteksi semua tanaman herbal di Indonesia
										</motion.div>
									</div>
									...
									<motion.div animate={{ y: [20, 0], opacity: [0, 1] }} transition={{ duration: 0.8, delay: 0.8 }} className="flex w-full flex-col gap-2">
										...
										<Input
											...
											className="placeholder:text-font-secondary border-font-secondary text-font-primary h-10 rounded-lg border-2 bg-white px-4 disabled:opacity-50"
										/>
									</motion.div>
									...
									<motion.div animate={{ y: [20, 0], opacity: [0, 1] }} transition={{ duration: 0.8, delay: 1 }} className="flex w-full flex-col gap-2">
										...
										<Input
											...
											disabled={!!loadingProvider}
											className="placeholder:text-font-secondary text-font-primary h-10 rounded-lg border-2 border-zinc-800/40 bg-white px-4 disabled:opacity-50"
										/>
									</motion.div>

									{/* Password */}
									<motion.div animate={{ y: [20, 0], opacity: [0, 1] }} transition={{ duration: 0.8, delay: 1.2 }} className="flex w-full flex-col gap-2">
									...
									{/* Tombol Register */}
									<motion.div animate={{ y: [20, 0], opacity: [0, 1] }} transition={{ duration: 0.8, delay: 1.4 }} className="flex w-full flex-col">
										<div className="mt-2 cursor-pointer w-full">
											<motion.div whileHover={!loadingProvider ? { scale: 1.02 } : {}} whileTap={!loadingProvider ? { scale: 0.98 } : {}}>
												<Button
													onClick={handleRegister}
													disabled={!!loadingProvider || !username || !email || !password}
													className="h-12 w-full cursor-pointer rounded-lg font-medium text-white disabled:opacity-50 disabled:cursor-not-allowed"
													style={{ background: "linear-gradient(to right, #537D5D, #73946B)" }}
												>
									...
									<motion.div animate={{ y: [20, 0], opacity: [0, 1] }} transition={{ duration: 0.8, delay: 0.6 }} className="flex items-center gap-6">
										...
										<motion.div animate={{ scale: [0, 1], opacity: [0, 1] }} transition={{ duration: 0.6, delay: 1 }} className="text-font-secondary text-base whitespace-nowrap">
											atau daftar dengan
										</motion.div>
										<motion.div animate={{ scaleX: [0, 1] }} transition={{ duration: 0.8, delay: 0.8 }} className="h-0.5 w-full rounded-lg bg-zinc-300" />
									</motion.div>
									...
									{/* Social Register */}
									<motion.div animate={{ y: [20, 0], opacity: [0, 1] }} transition={{ duration: 0.8, delay: 0.4 }} className="flex w-full flex-col gap-4 lg:flex-row">
										{/* Google */}
										<motion.div
											onClick={() => !loadingProvider && handleSocialLogin("google")}
										...
										{/* Facebook */}
										<motion.div
											onClick={() => !loadingProvider && handleSocialLogin("facebook")}
											whileHover={!loadingProvider ? { scale: 1.02 } : {}}
											whileTap={!loadingProvider ? { scale: 0.98 } : {}}
											className={`flex h-12 w-full items-center justify-center gap-2 rounded-lg border-2 border-zinc-800/40 bg-white ${loadingProvider ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
										>
